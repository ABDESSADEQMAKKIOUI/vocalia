...
import { AlertTriangle, LifeBuoy, Menu, RefreshCw } from "lucide-react";
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between border-b border-border/60 bg-background/70 px-4 py-2 backdrop-blur-md supports-[backdrop-filter]:bg-background/55">
      <div className="flex items-center gap-3">
        ...
        <Link href="/" className="text-lg font-bold md:hidden">Volira</Link>
      </div>
      {/* The upstream header carried a Slack invite to the Dograh community and
          a GitHub star badge for the upstream repository. Both are gone: on a
          white-labelled deployment they send this platform's customers to the
          project it is built on, which is the one place they should never land.
          Support goes through this deployment's own contact point instead, and
          the button disappears entirely when none is configured. */}
      <div className="flex items-center gap-3">
        {SUPPORT_EMAIL && (
