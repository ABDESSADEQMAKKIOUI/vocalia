/**
 * Volira Voice Widget
 * Embeddable voice call widget for Volira workflows
 * Version: 1.0.0
 */
...
(function() {
  ...
  // Origin of the <script> tag that loaded this widget. document.currentScript
  // is null once the script has finished executing, so it is captured eagerly.
  const LOADER_ORIGIN = (function () {
    try {
  ...
  function currentScriptOrigin() { return LOADER_ORIGIN; }

  // Widget configuration defaults
  const DEFAULT_CONFIG = {
    position: 'bottom-right',
    autoStart: false,
    // Fall back to the origin that served this script rather than to a
    // hard-coded vendor host: the widget is loaded from the deployment it
    // belongs to, so that origin is always the right backend. The upstream
    // default pointed every embed at api.dograh.com, which for a white-labelled
    // install meant sending a customer's call straight to another company.
  ...
  async function init() {
    ...
    if (!script) {
      console.error('Volira Widget: Script not found');
      return;
    }
    ...
    if (!token) {
      console.error('Volira Widget: No token found in script URL');
      return;
    }
    ...
    try {
      ...
      console.error('Volira Widget: Failed to fetch configuration', error);
      return;
    }
  ...
  function parseContextVariables(contextStr) {
    ...
    try {
      ...
      console.warn('Volira Widget: Invalid context variables', e);
      return {};
    }
  }
  ...
  /**
   * Create headless widget (no UI — host page drives everything via window.VoliraWidget API)
   */
  ...
  async function startCall() {
    ...
    try {
      ...
      console.error('Volira Widget: Failed to start call', error);
  ...
  async function fetchTurnCredentials() {
    if (!state.sessionToken) {
      console.warn('Volira Widget: No session token available for TURN credentials');
      return;
    }
  ...
  // Backwards-compatibility alias. The widget used to expose itself as
  // window.DograhWidget, and integrations embedded on customer sites before the
  // rename may still call it by that name. Both names point at the same object,
  // so either one drives the same widget instance.
  window.DograhWidget = window.VoliraWidget;
